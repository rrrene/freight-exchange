class FreightsController < RemoteController
  ...
  def create
    @freight = resource_class.new(params[:freight])
    @freight.user, @freight.company = current_user, current_company
    create!
  end
  
  def update
    @freight = resource_class.find(params[:id])
